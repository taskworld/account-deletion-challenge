  ...
  cors(request, response, async () => {
    ...
    if (request.body.pages[0].questions.length === 0) {
      return response.send(
        'Expected "pages[].questions" value to have at least 1 member'
      )
    }

    if (
      request.body.pages[0].questions.some(
        question => _.isString(question.id) === false
      )
    ) {
      return response.send(
        'Expected "pages[].questions[].id" field to be a string'
      )
    }
    ...
    if (
      request.body.pages[0].questions.some(
        question =>
          question.id !== MULTIPLE_CHOICE_ID && question.id !== COMMENT_ID
      )
    ) {
      return response.send(
        'Expected "pages[].questions[].id" value to either ' +
          MULTIPLE_CHOICE_ID +
          ' or ' +
          COMMENT_ID
      )
    ...
    if (
      request.body.pages[0].questions.length !==
      _.uniqBy(request.body.pages[0].questions, 'id').length
    ) {
      return response.send(
        'Expected "pages[].questions[].id" value to be unique'
      )
    }
    ...
    if (
      request.body.pages[0].questions.some(
        question =>
          _.isArray(question.answers) === false ||
          question.answers.length === 0 ||
          question.answers.some(answer => _.isPlainObject(answer) === false)
      )
    ) {
      return response.send(
        'Expected "pages[].questions[].answers" field to be an array of object'
      )
    }

    const multipleChoiceQuestion = request.body.pages[0].questions.find(
      question => question.id === MULTIPLE_CHOICE_ID
    )
      ...
      for (const answer of multipleChoiceQuestion.answers) {
        if (answer.choice_id) {
          if (_.difference(_.keys(answer), ['choice_id']).length > 0) {
            return response.send(
              'Expected "pages[].questions[].answers" value to be an object containing only "choice_id" field'
            )
          }

          if (_.includes(CHOICE_IDS, answer.choice_id) === false) {
            return response.send(
              'Expected "pages[].questions[].answers[].choice_id" value to be one of ' +
                CHOICE_IDS.join(', ')
            )
          }
        } else {
    ...
    const freeTextQuestion = request.body.pages[0].questions.find(
      question => question.id === COMMENT_ID
    )
    if (freeTextQuestion) {
      if (freeTextQuestion.answers.length !== 1) {
        return response.send(
          'Expected "pages[].questions[].answers" value to have exactly 1 member'
        )
      }

      if (_.isEqual(_.keys(freeTextQuestion.answers[0]), ['text']) === false) {
        return response.send(
          'Expected "pages[].questions[].answers" value to be an object containing only "text" field'
        )
      }

      if (
        _.isString(freeTextQuestion.answers[0].text) === false ||
        freeTextQuestion.answers[0].text.length === 0
      ) {
        return response.send(
